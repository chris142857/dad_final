...
class HiddenObjects(nn.Module):
    ...
    def model(self):
        ...
        for t in range(self.T):
            ...
            # # {FB}
            # y_0 = observation_sample(f"y{t + 1}_0", dist.Normal(mean[:,0].unsqueeze(dim=-1), sd).to_event(1))
            # y_1 = observation_sample(f"y{t + 1}_1", dist.Normal(mean[:,1].unsqueeze(dim=-1), sd).to_event(1))
    ...
    def eval(self, n_trace=100, theta=None, verbose=True):
        ...
        with torch.no_grad():
            for i in range(n_trace):
                ...
                # # # {FB} 1D plot
                # # fig, ax = plt.subplots()
                # # for i in range(self.p): ax.plot(run_df["order"], run_df[f"xi_{i}"])
                ...
                #
                # # {FB} 2D plot
                # plt.scatter(run_df["xi_0"], run_df["xi_1"], c=run_df["order"], cmap='viridis', alpha=0.5)
                # plt.scatter(true_theta[0,0], true_theta[0,1], c='b', alpha=0.5)
                # #plt.scatter(true_theta[1,0], true_theta[1,1], c='b', alpha=0.5)
                # #plt.show()
...
def single_run(
    ...
    *adam_betas, adam_weight_decay = adam_betas_wd
    ...
    # # {FB} using fixed true_theta
    # fix_theta = torch.tensor([[2.00]],device='cuda:0')
    # ho_model.eval(theta=fix_theta)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mlflow-experiment-name", default="face_finding-feedforward-continuous", type=str
    )
